...
describe('/authentications endpoint', () => {
  ...
  describe('when POST /authentications', () => {
    ...
    it('should response 401 if password wrong', async () => {
      ...
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message).toEqual('inputed credentials are wrong');
    });

    it('should response 400 if login payload ' +
      ...
      expect(responseJson.message)
          .toEqual('must contain username and password');
    });
    ...
    it('should response 400 if login payload wrong data type', async () => {
      ...
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message)
          .toEqual('username and password must be string');
    });
  });
  ...
  describe('when PUT /authentications', () => {
    ...
    it('should return 400 payload not contain refresh token', async () => {
      ...
      expect(response.statusCode).toEqual(400);
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message).toEqual('must contain refresh token');
    });
    ...
    it('should return 400 if refresh token ' +
        'not registered in database', async () => {
      // Arrange
      const server = await createServer(container);
      ...
      expect(responseJson.status).toEqual('fail');
      expect(responseJson.message)
          .toEqual('refresh token is not registered in database');
    });
  });
  ...
  describe('when DELETE /authentications', () => {
    ...
    it('should response 400 if refresh token ' +
        'not registered in database', async () => {
      // Arrange
      const server = await createServer(container);
      const refreshToken = 'refresh_token';
