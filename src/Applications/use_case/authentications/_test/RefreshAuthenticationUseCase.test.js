...
describe('RefreshAuthenticationUseCase', () => {
  it('should throw error if use case payload ' +
    ...
    // Action & Assert
    await expect(refreshAuthenticationUseCase.execute(useCasePayload)).rejects
        .toThrowError(
  ...
  it('should orchestrating ' +
      'the refresh authentication action correctly', async () => {
    // Arrange
    const useCasePayload = {
    ...
    mockAuthenticationTokenManager.decodePayload = jest
        .fn(() => Promise.resolve({username: 'dicoding', id: 'users-123'}));
    mockAuthenticationTokenManager.createAccessToken = jest
        .fn(() => Promise.resolve('some_new_access_token'));
    ...
    expect(mockAuthenticationTokenManager.createAccessToken)
        .toBeCalledWith({username: 'dicoding', id: 'users-123'});
